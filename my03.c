// single comment

/* 
   multiline comment
   " " double quote, ' ' single quote, # hash, / slash, \ backslash
   : colon, ; semicolon, ( ) round bracket, [ ] sqare bracket
   { } curly bracket, < > angle bracket angle bracket, | Pipe, & ampersand
   ` backtick (Alt+9+6), ^ caret
*/

#include "stdio.h" //หรือ <stdio.h>

int main(){
    //แสดงเฉพาะที่อยู่ใน ""แรกเท่านั้น โดยเขียนอย่างไงแสดงอย่างนั้น ***ยกเว้นรหัสแบ็คสเลช format spec/format code
    //แต่หากมีอะไรต่อจาก ""แรก จะต้องนำมาแสดงผลที่ "" แรกเสมอ โดยใช้ format spec/format code
    /*
      printf("Hello....\n");

      printf("AAA %d BBB %s %d^_^\n", 555,"Wow wow wow", 100+200);

      printf("Hi\n\n\nHey\tHoo\t\t\tHow\\");
    */

    return 0;
}

